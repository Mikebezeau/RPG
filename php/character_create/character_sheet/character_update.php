<?php
	...
	//***********************
	// CHANGE TO NEW ARMOR
	// if new armor has been chosen. make sure value is not -1 (default value)
	...
	// if new shield has been chosen. make sure value is not -1 (default value)
	...
	if(isset($_POST["arr_weaponid"])) {
		...
		$equip_weapon = array();
		foreach($_POST["arr_weaponid"] as $i => $value) {
			if(isset($_POST["equip_weapon$i"])) { $equip_weapon[] = 1; } else { $equip_weapon[] = 0; }
		}
		
		$offhand_weapon = array();
		foreach($_POST["arr_weaponid"] as $i => $value) {
			if(isset($_POST["offhand_weapon$i"])) { $offhand_weapon[] = 1; } else { $offhand_weapon[] = 0; }
		}
		...
		foreach ($arr_weaponid as $i => $value)
		{
			$query = "UPDATE CharacterWeapon
					SET Enhance=".$arr_weaponenhance[$i].", MW=".$weapon_mw[$i].", Equipped=".$equip_weapon[$i].", OffHand=".$offhand_weapon[$i].", TwoHand=".$arr_twohand[$i].", UseDex=".$use_dex[$i].", Flurry=".$flurry[$i].", 
						CustomName='".$arr_customname[$i]."', HitBonus=".$arr_hitbonus[$i].", DamageBonus=".$arr_damagebonus[$i].", CritRangeBonus=".$arr_crit_range_bonus[$i].", 
						Notes='".$arr_weaponnotes[$i]."'  
	...
	//insert new weapon if one was chosen. make sure value is not -1 (default value)
	if(isset($_POST["weaponid_new"]) && $_POST["weaponid_new"] != -1) {
		...
		$query = "INSERT INTO CharacterWeapon (WeaponID, CharacterID, Enhance, MW, Equipped, TwoHand, UseDex, Flurry, CustomName, HitBonus, DamageBonus, CritRangeBonus)
					VALUES (".$weaponid_new.", ".$character_id.", ".$weaponenhance_new.", FALSE, FALSE, ".$two_hand.", FALSE, FALSE, '', 0, 0, 0)";
		// Perform Query
		$result = mysqli_query($link,$query);
	}
	...
	if(isset($_POST["submit_select_feats"])) {
		mysqli_close($link);
		header('refresh:0; url=../../../character_select_feat.php?character_id='.$character_id);
		exit;
	}
	elseif (isset($_POST["submit_edit_ability"])) {
		mysqli_close($link);
		header('refresh:0; url=../../../character_edit_ability.php?character_id='.$character_id);
		exit;
	}
	...
	for($i = 1; $i <= $highest_class_id; $i++) {
 		//inside double quotes "submit_spells" and $i will be parsed and concatenated
       	if(isset($_POST["submit_spells$i"])) {
			...
			mysqli_close($link);
			header('refresh:0; url=../../../character_select_spell.php?character_id='.$character_id.'&learned_class_id='.$i);
			exit;
		}
	}
